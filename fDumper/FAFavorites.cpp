...
int CFAFavorites::Action(const DownloadContext& ctx)
{
...
std::vector<FASubmission> CFAFavorites::GetFavoritesGallery()
{
	...
	//		sprintf_s(urlbuff, "%s/user/%s/favorites.json?full=1&sfw=1&next=%d", g_api.c_str(), m_uHandle.c_str(), favorite_id);
	//	else
	//		sprintf_s(urlbuff, "%s/user/%s/favorites.json?full=1&next=%d", g_api.c_str(), m_uHandle.c_str(), favorite_id);

...
bool CFAUpdater::Argument(arg_t & arg)
{
	...
	else if (!s.compare(L"--no-favorites"))
	{
		m_nofaves = true;
		return ++arg.i <= arg.c;
	}
	else if (!s.compare(L"--favorites-only"))
	{
		m_favesonly = true;
		return ++arg.i <= arg.c;
	}
...
std::vector<int> CFAUpdater::GetUserFavoriteGalleryPages(std::string user, int rating)
{
	...
	while (true)
	{
		...
		if (rating == SFW_ONLY)
			sprintf_s(urlbuff, "%s/user/%s/favorites.json?full=1&sfw=1&next=%d", m_api.c_str(), user.c_str(), favorite_id);
		else
			sprintf_s(urlbuff, "%s/user/%s/favorites.json?full=1&next=%d", m_api.c_str(), user.c_str(), favorite_id);
